...
class SocketServer extends EventEmitter {
    ...
    _pushLatestTrades(socket){
        this.coinpusher.css.forEach(cs => {
            ...
            let trades = [];
            if(cs.dataset.length <= MAX_PUSH_ON_CON_COUNT){
                trades = cs.dataset.slice(0);
            } else {
                for(let i = cs.dataset.length - 1; i >= (cs.dataset.length - 1 - MAX_PUSH_ON_CON_COUNT); i--){
                    trades.push(cs.dataset[i]);
                }
                trades.reverse();
            }
