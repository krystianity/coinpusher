...
const ETLS = {
    [CURRENCY.ETH_EUR]: {
        ...
        main: row => { //before training (is mapped)
            ...
            trades.forEach(trade => {
                row.x.push(trade.price / 1000);
                row.x.push(trade.amount / 1000);
                row.x.push(TIMESTAMP_TO_HOUR(trade.timestamp) / 24);
                row.x.push(!trade.type ? 1 : 0);
                row.x.push(trade.type ? 1 : 0);
            });
    ...
    [CURRENCY.BTC_EUR]: {
        predict: input => { //before prediction (is called)
            return input.map(data.price / 10000);
        },
    ...
    [CURRENCY.LTC_EUR]: {
        predict: input => { //before prediction (is called)
            return input.map(data.price / 100);
        }, 
