...
const ETL_ROW = data => data.price / 1000;
...
class Coinpusher {
    ...
    _onTrade(data = {}){
        ...
        if(this.buffers[currency].length >= 10){ //TODO 10 must not be hard coded
            if(this.nets[currency]){
                data.predicted = this.nets[currency].predict(this.buffers[currency].map(ETL_ROW));
                debug("predicting trades", data.predicted);
            }
        }
    ...
    _onStep(data = {}){
        ...
        if(this.nets[currency]){
            data.predicted = this.nets[currency].predict(collected.x.map(ETL_ROW));
            debug("predicting step", data.predicted);
        }
