...
class Coinpusher {
    ...
    _getDatasetForCurrency(currency){
        ...
        for(let i = 0; i < this.css.length; i++){
            if(this.css[i].currency === currency){
                return this.css[i].dataset.slice(0);
            }
        }
    ...
    _onStep(data = {}){
        ...
        if(this.nets[currency]){
            ...
            data = Object.assign({}, data, {predicted});
            debug("predicted steps");

            //TODO dataset breaks into NULLS WTF
            //TODO network only predicts NULL WTF
            //TODO currencies need to be normalized btc is 1k and eth is 100 etc.
        }
